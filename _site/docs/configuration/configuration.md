...
| execFileSystemType                | `CFC`, _`CFC`_; `FUSE`        |                       | Execroot presentation. `FUSE` eagerly downloads and references every input in the filesystem CAS before execution, then projects the directory index with reads directed at the local CAS files. It does not materialize input files or directory trees. FUSE requires Linux and does not support `execOwner`/`execOwners`. |
