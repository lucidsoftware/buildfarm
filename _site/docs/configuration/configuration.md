...
| linkedInputDirectories            | List of Strings, _empty_      |                       | **Deprecated.** Ignored. The input link exclusion computation automatically determines which directories can be symlinked based on output paths. To force specific directories to remain real, use `linkedInputExclusionPatterns`. |
| linkedInputExclusionPatterns      | List of Strings, _empty_      |                       | Regex patterns matched against each input directory's path relative to the input root. A matching directory is kept as a real directory instead of being symlinked to its CAS tree; matches merge with the auto-computed exclusion set (a directory is excluded from linking if it is an output-path ancestor OR matches a pattern). Migration from the deprecated `linkedInputDirectories`: that setting was an *inclusion* regex (directories matching it were eligible for linking), so rewrite your inclusion regex into the complementary *exclusion* pattern — e.g. the old default `^(?!external$).*$` (link everything except `external`) becomes the exclusion pattern `external` (keep only `external` real). Each pattern is matched in full (anchored), not as a substring. An invalid regex fails worker startup. Empty by default, which preserves the auto-computed behavior. |
