...
public abstract class NodeInstance extends InstanceBase {
  ...
  private static boolean directoryTreeContainsSymlinks(
    ...
    if (directory.getSymlinksCount() > 0) {
      return true;
    }
    for (DirectoryNode directoryNode : directory.getDirectoriesList()) {
      build.bazel.remote.execution.v2.Digest digest = directoryNode.getDigest();
      if (!visited.add(digest)) {
        continue;
      }
      Directory subDirectory = directoryFromDigest(digest, directoriesIndex);
      if (subDirectory != null
          && directoryTreeContainsSymlinks(subDirectory, directoriesIndex, visited)) {
        return true;
      }
    }
    return false;
      ...
      if (pathDigests.contains(directoryDigest)) {
        ...
        boolean canReuseValidation =
            prevDepth != null
                ...
                && subDirectory != null
                && !directoryTreeContainsSymlinks(subDirectory, directoriesIndex, new HashSet<>());
