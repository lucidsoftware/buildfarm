...
public class ExecutionWrapperUtils {
  ...
  public static void applyExecutionWrappers(
      ImmutableList.Builder<String> arguments,
      Iterable<ExecutionPolicy> policies,
      Claim claim,
      Iterable<Property> platformProperties,
      boolean applyPrioritized) {
    Map<String, Interpolator> interpolations = createInterpolations(claim, platformProperties);

    for (ExecutionPolicy policy : policies) {
      if (policy.isPrioritized() == applyPrioritized && policy.getExecutionWrapper() != null) {
